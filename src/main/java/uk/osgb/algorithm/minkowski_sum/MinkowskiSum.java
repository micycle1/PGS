...
public class MinkowskiSum {
	...
	private static Geometry expansionShell(Polygon src, Coordinate[] refCoords, boolean isRefConvex) {
		...
		ArrayList<LinearRing> ringVec = null;
		...
		if (shellHoles != null) {
			...
			for (LinearRing shellHole : shellHoles) {
				Point ep = gf.createPolygon(shellHole).getInteriorPoint();
				if (!geomNoHole.contains(ep)) {// holes outside original geometry, generated by expansion, should be
					...
					if (ringVec == null) {
						ringVec = new ArrayList<>();
					}
					ringVec.add(shellHole);
				}
			}
		}
		// assemble the result
	...
	private static Geometry shrinkHole(Polygon src, Coordinate[] refCoords, boolean isRefConvex) {
		...
		ArrayList<LinearRing> ringVec = new ArrayList<>(shellHoles.length);
