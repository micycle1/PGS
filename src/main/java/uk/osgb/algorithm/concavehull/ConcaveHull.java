...
public class ConcaveHull {
	...
	public ConcaveHull(Collection<Geometry> geomCol, double densifyTol) {
		if (geomCol != null && geomCol.size() > 0) {
			...
			if (densifyTol > 0.0) {
				gc = new ArrayList<Geometry>(geomCol.size());
	...
	private void initArray(Coordinate[] coordArray, GeometryFactory gf) {
		this.gf = gf;
		Collection<Coordinate> coordCol = new ArrayList<Coordinate>(coordArray.length);
	...
	public ArrayList<Geometry> getConcaveHullBFS(TriangleChecker triChecker, boolean allowMultiParts,
		...
		if (triChecker != null) {
			...
			ArrayList<DLCirList> rltHulls = new ArrayList<DLCirList>(); // finished hulls
			...
			ArrayList<Geometry> rtn = new ArrayList<Geometry>(hulls.size() + rltLS.size() + rltPt.size());
	...
	public Collection<Geometry> getConcaveHullMetric(TriangleChecker triChecker, TriMetricLength m,
		...
		if (triChecker != null) {
			...
			ArrayList<Geometry> rtn = new ArrayList<Geometry>(hulls.size() + rltLS.size());
			for (DLCirList<Coordinate> h : rltHulls) {
				Coordinate[] coords = new Coordinate[h.size() + 1];
				int cnt = 0;
				DLNode<Coordinate> node = h.getNode();
	...
	public Geometry getConcaveHullWithHolesChi(TriCheckerChi tcChi) {
		...
		List triCoords = sd.getTriangleCoordinates(false);
		Collection<Geometry> holeCol = new ArrayList<Geometry>();
		for (Object obj : triCoords) {
			Coordinate[] coords = (Coordinate[]) obj;
	...
	class DLCirList<T> {
		...
		/**
		 * split the current circular list into two by link n1 and n2; current list
		 * contains edge n1-n2 and new list containing edge n2-n1 will be returned
		 * 
		 * @param n1
		 * @param n2
		 * @return
